...
export class FsNuevoComponent implements OnInit, AfterViewInit {
  ...
  /**
   * Agrega etiquetas HTML sobre el gráfico (para sectores o activos, según corresponda).
   */
  addHtmlLabels(chart: Chart, contenedor: any, es_sector: boolean) {
    const container = contenedor;
    container.innerHTML = '';
  ...
  /**
   * renderChart() actualizado:
   * – Extrae el arreglo "valor_cuota" del JSON.
   * – Genera la lista de etiquetas tomando cada "periodo".
   * – Para cada posición (índice) en el arreglo "valores" se crea un dataset.
   * Así, cada "punto" se representa con una línea separada en el gráfico.
   */
  renderChart(): void {
  ...
  /**
   * Función para formatear números con separador de miles y límite de decimales.
   */
  formatoNumberMiles(x: any, decimalLimit: number = 2) {
    if (x && !isNaN(Number(x))) {
  ...
  /**
   * Función auxiliar para generar un color hexadecimal aleatorio.
   */
  ...
  /**
   * Método para generar las imágenes de los gráficos y crear el PDF.
   */
  async generar() {
    let imgSectores: any;
    let imgActivos: any;
